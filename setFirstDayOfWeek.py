...
try:
    ...
    print('using QVariant 1 in the GUI')
  
from etmQt.etmData import has_sip, qt_version
if not has_sip or not qt_version:
    print("""
SIP, Qt and PyQt are required for the etm-qt GUI but could not be imported.
The command line version of etm can be used without these imports. Enter
  
    etm_qt ?
  
at a command prompt to see a list of the available commands.
""")
    sys.exit()
...
if qt_version == 5:
    ...
    # THIS IS NECESSARY NO MATTER WHAT PEP8 SAYS!!!
    from etmQt.ui5 import etm_rc
...
from etmQt.etmData import (
    ...
    python_version, qt_version, windoz)
...
class DictionaryCompleter(QCompleter):
    def __init__(self, parent=None, options={}):
        ...
        words = []
        try:
            f = open(options['auto_completions'], "r")
        ...
        QCompleter.__init__(self, words, parent=parent)
...
class CompletionTextEdit(QTextEdit):
    def __init__(self, parent=None, options={}):
        ...
        self.setStyleSheet(
            'font-size: %dpt' % options['fontsize'])
        self.completer = None
    ...
    def keyPressEvent(self, event):
        ...
        # the following hack is for qt5 to leave focus with the editor
        self.setFocus()
        self.parent.raise_()
...
Adding item to {1} failed - aborted removing item from {2}""".format(
    ...
    def doIt(self, mode):
        ...
        if ret == 1:
            ...
            if doit:
                new_dt = parse(
                    parse_datetime(datestr)).replace(
                    tzinfo=tzlocal()).astimezone(
                    gettz(self.hsh['z'])).replace(tzinfo=None)
                if new_dt:
                    old_dt = parse(
                        ...
                        tzinfo=tzlocal()).astimezone(
                        gettz(self.hsh['z'])).replace(tzinfo=None)
                    # if old_dt is in @+, remove it
                    if '+' in self.hsh and old_dt in self.hsh['+']:
                            self.hsh['+'].remove(old_dt)
                    # else old_dt must be from the rrule, make sure it's in @-
                    elif '-' not in self.hsh or old_dt not in self.hsh['-']:
                        self.hsh.setdefault('-', []).append(old_dt)
                    # make sure new_dt is in @+
                    if '+' not in self.hsh or new_dt not in self.hsh['+']:
                        self.hsh.setdefault('+', []).append(new_dt)
                    if '-' in self.hsh:
                        self.hsh['-'].sort()
                    if '+' in self.hsh:
                        self.hsh['+'].sort()
            ...
            if u'+' in self.hsh:
                ...
                for d in hsh_rev['+']:
                    if d < dt:
                        ...
                        tmp_cpy.append(d)
                hsh_rev['+'] = tmp_rev.sort()
                hsh_cpy['+'] = tmp_cpy.sort()
            if u'-' in self.hsh:
                tmp_rev = []
                tmp_cpy = []
                for d in hsh_rev['-']:
...
class EditForm(QMainWindow, Edit_Dialog):
    def __init__(
        ...
        completer = DictionaryCompleter(parent=self, options=self.options)
        self.editor = CompletionTextEdit(parent=self, options=self.options)
...
background: transparent; font-size: 14pt; font-family: CourierNewPSMT"""))
    ...
    def reportRemove(self):
        ...
        if text == curr_text:
            # only remove the item if the displayed value is the same as
            # the stored value
...
""" % (datestr, res))
    ...
    def timer_finish(self, create=True):
        ...
        if self.timer_hsh:
            ...
            if form:
                ...
                form.editor.document().setModified()
            ...
            inital_entry = "~ %s @s %s @e %s" % (
                self.timer_summary,
                fmt_datetime(finish_time, self.options),
    ...
    def timer_start(self, hsh={}):
        ...
        if ok:
            self.timer_hsh = {}
            self.timer_status_hsh = {}
            timer_hsh = str2hsh(str(summary), options=self.options)[0]
            self.timer_summary = timer_hsh['_summary']
            self.starttime = starttime
            self.timer_status_hsh['summary'] = timer_hsh['_summary']
            if 'e' in timer_hsh:
                time_delta = timer_hsh['e']
            else:
                time_delta = 0 * oneminute
            self.timer_delta = time_delta
            if ('running' in self.options['action_status'] and
                    self.options['action_status']['running']):
                tcmd = expand_template(
                    self.options['action_status']['running'],
...
Click here or press Shift Ctrl-T to stop the timer and record the action."""))
    ...
    def createItem(self, title=None, text="", modified=False):
        ...
        form = getEditForm(parent=self, instance='edit', receiver=self
                           .itemChanged)
        if form:
...
def main(etmdir=''):
    ...
    if use_locale:
        if windoz:
            locale.setlocale(locale.LC_ALL, str(use_locale[0][0]))
        else:
    ...
    # Either of the following works for pyqt4 but not pyqt5
    # app.setWindowIcon(QIcon(":/etm.png"))
    icon = QIcon()
    icon.addPixmap(QPixmap(":etm.png"), QIcon.Normal, QIcon.Off)
